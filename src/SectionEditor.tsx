        ...
        return (
            <PanelSection title={"[" + section.Name + "]"} >
                {options.map((option, index) => {
                    if (modeLevel >= option.ModeLevel) return (
                        <FieldEditor field={option} onChange={(updatedOption) => handleOptionChange(index, updatedOption)}
                            updateHelpText={(field: KeyValuePair) => { updateHelpText(field) }}
                        />
