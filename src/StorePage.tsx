...
export const StorePage: VFC<{ serverAPI: ServerAPI; }> = ({ serverAPI }) => {
    ...
    useEffect(() => {
        serverAPI.callPluginMethod<{}, GameData[]>("get_game_data", { filter: searchQuery }).then(data => {
    ...
    const onInit = async () => {
        serverAPI.callPluginMethod<{}, GameData[]>("get_game_data", { filter: "" }).then(data => {
            setGames(data.result as GameData[]);
        });
    ...
    return (
        <div style={{ margin: "50px", color: "white" }}>
            <Focusable style={{
                ...
                <TextField placeholder="Search" value={searchQuery} onChange={(e: any) => setSearchQuery(e.target.value)} />
            </Focusable>
